...
export const LoaderPageElement = {
  type: 'loaderpage',
  label: 'Loader',
};

export function LoaderPageContent() {
  const [copy, setCopy] = useState(false);
  const [copyInstall, setCopyInstall] = useState(false);
  const [selectedTab, setSelectedTab] = useState('preview');

const codeString = `import XLoader from "@/xcomponents/xloader"

export function ButtonDemo() {
  return <XButton variant="primary">Button</XButton>
}`;

const installString=`npx xsans add xloader`
  ...
  return (
    <div className="w-full h-full p-9 flex flex-col gap-9">
      ...
      <div className="w-[850px] h-[150px] bg-gradient-to-b from-[#dddddd] to-[#747474] text-white p-5 rounded-lg flex flex-col gap-2">
        <h1 className="text-3xl font-notosans font-bold text-slate-900 ">Loader</h1>
        <p className="font-notosans text-slate-800 font-medium">Displays a loader or a component that looks like a loader</p>
      ...
      <div className="flex flex-col gap-5 w-[50%]">
         ...
         <div className="w-full h-[250px] flex flex-col gap-3 mt-5">
           <lable className='font-notosans text-xl font-semibold'>Simple Loader</lable>
          ...
          {selectedTab === 'preview' && (
            <div className="flex bg-[#17171b] items-center justify-center mt-2 w-full h-[250px] rounded-lg">
              <XLoader type="spinner" size="md" color="blue" />
            </div>
          )}
          {/* code section */}
          {selectedTab === 'code' && (
